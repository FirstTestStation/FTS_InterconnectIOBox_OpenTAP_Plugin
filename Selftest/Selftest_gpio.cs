    ...
    public class Selftest_gpio : ResultTestStep
    {
        ...
        public override void Run()
        {
            ...
            if (Selectgp != GpioPins.None) // If Action need to be performed on GPIO on Basic Configuration
            {
                ...
                {
                    ConfigureBasicGpio("SetPad", (byte)Selectgp, SetPads.Value, false, Validate);
                    ConfigureBasicGpio("SetPadgp", (byte)Selectgp, SetPads.Value, true, Validate);
                }

                if (GetPads.IsEnabled == true)
                {
                    ConfigureBasicGpio("GetPad", (byte)Selectgp, GetPads.Value, true, Validate);
                }

                if (Getfct.IsEnabled == true)
                {
                    ConfigureBasicGpio("Getfunction", (byte)Selectgp, Getfct.Value, true, Validate);
                }
