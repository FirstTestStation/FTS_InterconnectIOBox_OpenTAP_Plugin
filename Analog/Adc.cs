...
using static InterconnectIOBox.Analog.ADCReadVolt;
...
namespace InterconnectIOBox.Analog
{
	public abstract class ADCReadVolt : ResultTestStep
	{
		...
		[Display("ADC Source", Order: 0, Group: "ADC Source Selection", Description: "Choose the ADC source, internal or external, to use.")]
		public AdcSource SelectedAdc { get; set; }


		[Display("Low Limit", Order: 1, Group: "Test Limit", Description: "Set the lower and upper limits for the measurement.")]
		public double LowerLimit { get; set; }

		[Display("High Limit", Order: 2, Group: "Test Limit")]
		public double UpperLimit { get; set; }

		[Display("Publish Results", Order: 3, Group: "Options", Description: "If checked, publish the measured ADC result. If unchecked, the measurement still runs and affects the verdict, but no result is published.")]
		public bool PublishResults { get; set; } = true;
		#endregion

	}

	[Display(Groups: new[] { "FTS_Interconnect", "Analog" }, Name: "ADC", Description: "Read the voltage from the Pico Analog-to-Digital Converter (ADC) and verify if it falls within the expected range." +
	...
	{
		[Output]
		[Display("Measure:", Order: 4, Description: "The measured ADC value with its unit, shown after the step has run.")]
