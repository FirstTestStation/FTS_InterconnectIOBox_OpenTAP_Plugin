...
namespace InterconnectIOBox.Analog
{
	...
	public abstract class SingleRelay : ResultTestStep
	{
		...
		[Display("Relay 5V_DUT", Order: 5, Group: "Relays Control", Description: "Set the 5V_DUT power relay state to Open, Close, or NoAction." +
		" You can also read the relay state, close the relay and verify if it is closed, or open the relay and verify if it is open.")]
	...
	#endregion
	[Display(Groups: new[] { "FTS_Interconnect", "Analog" }, Name: "Isolated Relay Control", Description: "SCPI command to set the state of relays LPR1, LPR2, HPR1, SSR1, and 5V_DUT." +
		" You can also read the relay state, close the relay and verify if it is closed, or open the relay and verify if it is open.")]
	public class RelayAction : SingleRelay
	{
		...
		public override void Run()
		{
			...
			UpgradeVerdict(Verdict.Pass); // default value in case all relays are NoAction
		...
		private void HandleRelay(string relayName, RelayCondition condition)
		{
			...
			switch (condition)
			{
				...
				case RelayCondition.Read_Only:
					Log.Info($"Reading relay {relayName} state only, no action sent");
					break;
			...
			if (!string.IsNullOrEmpty(command) && condition != RelayCondition.Read_Only) // if a write command exists
			{
			...
			if (condition == RelayCondition.Open_Only || condition == RelayCondition.Close_Only)
			{
			...
			if (!isValid)
			{
				Log.Error($"Invalid response from relay {relayName}: {response}");
			}
