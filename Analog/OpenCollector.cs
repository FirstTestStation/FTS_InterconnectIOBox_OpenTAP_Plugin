...
namespace InterconnectIOBox.Analog
{
	...
	public abstract class OpenCollector : ResultTestStep
	{
		...
		public enum OCRead
		{
			...
			CheckClose,    // Send command to check the transistor is driven (closed)
			CheckOpen      // Send command to check the transistor is open
		}

		[Display("Read Open Collector #1", Order: 2, Group: "OC Transistor Validation", Description: "Validate the state of the transistor: no read, check closed, or check open.")]
		public OCRead OC1r { get; set; }


		[Display("Read Open Collector #2", Order: 2.1, Group: "OC Transistor Validation", Description: "Validate the state of the transistor: no read, check closed, or check open.")]
		public OCRead OC2r { get; set; }


		[Display("Read Open Collector #3", Order: 2.2, Group: "OC Transistor Validation", Description: "Validate the state of the transistor: no read, check closed, or check open.")]
		public OCRead OC3r { get; set; }

		[Display("Publish Results", Order: 3, Group: "Options", Description: "If checked, publish the read-back verification results. If unchecked, verification still runs and affects verdict, but no result is published.")]
	...
	[Display(Groups: new[] { "FTS_Interconnect", "Analog" }, Name: "Open Collector Transistor Control", Description: "SCPI command to control the three open-collector transistors and read back their status.")]
